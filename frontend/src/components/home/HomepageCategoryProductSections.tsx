...
import { HomepageCategoryProductSection } from '@/lib/types';
import { Container } from '@/components/layout/Container';
import { ProductGrid } from '@/components/products/ProductGrid';
import { HomepageProductRail } from '@/components/home/HomepageProductRail';

interface HomepageCategoryProductSectionsProps {
  sections?: HomepageCategoryProductSection[];
}

function getCategoryStoreHref(slug: string | null | undefined): string | null {
  if (!slug) return null;
  return `/store?category=${encodeURIComponent(slug)}`;
}

export function HomepageCategoryProductSections({ sections = [] }: HomepageCategoryProductSectionsProps) {
  ...
  return (
    <div className="bg-white">
      {visibleSections.map((section) => {
        ...
        const categoryHref = getCategoryStoreHref(section.category?.slug);
