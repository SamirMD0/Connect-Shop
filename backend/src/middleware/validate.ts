...
function isSafeHomepageSectionKey(value: string): boolean {
  return (
    HOMEPAGE_SECTION_KEYS.includes(value as any) ||
...
export const addressRules: ValidationChain[] = [
  body("label")
    ...
    .trim()
    .isLength({ max: 80 })
    .withMessage("Label must be under 80 characters")
    .escape(),
  body("recipientName")
    .trim()
...
export const productQueryRules: ValidationChain[] = [
  ...
  query("sort")
    ...
    .withMessage(
      "sort must be one of: price_asc, price_desc, newest, rating, popular",
    ),
  query("brand")
    .optional()
    .trim()
...
export const productSlugRules: ValidationChain[] = [
  param("slug").trim().isSlug().withMessage("Invalid product slug"),
];
...
export const adminCategoryRules: ValidationChain[] = [
  ...
  body("depth")
    .optional()
    .isInt({ min: 0 })
    .withMessage("Depth must be 0 or greater"),
];
...
export const adminProductRules: ValidationChain[] = [
  ...
  body("gallery_images.*.image_url")
    .optional()
    .trim()
    .custom(isImageReference)
    .withMessage("Gallery image must be a valid URL or site image path"),
  body("gallery_images.*.alt_text")
    .optional({ nullable: true })
    .trim()
    .isLength({ max: 255 }),
  ...
  body("variants.*.image_url")
    ...
    .trim()
    .custom((value) => !value || isImageReference(value))
    .withMessage("Variant image must be a valid URL or site image path"),
];
...
export const carouselSlideRules: ValidationChain[] = [
  ...
  body("link_url")
    ...
    .withMessage("Link URL must be a safe URL or site path"),
  body("button_text")
    .optional({ nullable: true })
    .trim()
    .isLength({ max: 100 }),
...
const homepageSectionFields: ValidationChain[] = [
  ...
  body("eyebrow")
    ...
    .trim()
    .isLength({ max: 255 })
    .withMessage("eyebrow must be under 255 characters"),
  body("button_text")
    .optional({ nullable: true })
    .trim()
    .isLength({ max: 100 })
    .withMessage("button_text must be under 100 characters"),
  body("button_link")
    .optional({ nullable: true })
    .trim()
    .isLength({ max: 2048 })
  ...
  body("background_image_url")
    ...
    .custom((value) => !value || isImageReference(value))
    .withMessage(
      "background_image_url must be a safe image URL or site image path",
    ),
  body("sort_order")
    .optional()
    .isInt({ min: 0 })
...
export const homepageItemIdRules: ValidationChain[] = [
  param("id")
    .isUUID()
    .withMessage("Homepage section item ID must be a valid UUID"),
];
...
const forbiddenHomepageBlockFields: ValidationChain[] = [
  body("display_order")
    .not()
    .exists()
    .withMessage("display_order is controlled by move up/down actions"),
  body("sort_order")
    .not()
    .exists()
    .withMessage("sort_order is not supported for homepage blocks"),
  body("metadata")
    .not()
    .exists()
    .withMessage("metadata is not supported for homepage blocks"),
  body("image_url")
    .not()
    .exists()
    .withMessage("image_url is not supported for homepage blocks"),
  body("background_image_url")
    .not()
    .exists()
    .withMessage("background_image_url is not supported for homepage blocks"),
  body("button_link")
    .not()
    .exists()
    .withMessage("button_link is not supported for homepage blocks"),
  body("link_url")
    .not()
    .exists()
    .withMessage("link_url is not supported for homepage blocks"),
  body("url")
    .not()
    .exists()
    .withMessage("url is not supported for homepage blocks"),
  body("raw_url")
    .not()
    .exists()
    .withMessage("raw_url is not supported for homepage blocks"),
];
...
export const homepageBrandProductSectionIdRules: ValidationChain[] = [
  param("id")
    .isUUID()
    .withMessage("Homepage brand product section ID must be a valid UUID"),
];
...
const forbiddenHomepageBrandProductFields: ValidationChain[] = [
  ...
  body("sort_order")
    .not()
    .exists()
    .withMessage("sort_order is not supported for brand product sections"),
  body("metadata")
    .not()
    .exists()
    .withMessage("metadata is not supported for brand product sections"),
  body("image_url")
    .not()
    .exists()
    .withMessage("image_url is not supported for brand product sections"),
  body("background_image_url")
    .not()
    .exists()
    .withMessage(
      "background_image_url is not supported for brand product sections",
    ),
  body("button_link")
    .not()
    .exists()
    .withMessage("button_link is not supported for brand product sections"),
  body("link_url")
    .not()
    .exists()
    .withMessage("link_url is not supported for brand product sections"),
];
...
const homepageBrandProductSectionFields: ValidationChain[] = [
  ...
  body("product_limit")
    ...
    .bail()
    .custom((value) =>
      HOMEPAGE_BRAND_PRODUCT_LIMITS.includes(Number(value) as any),
    )
    .withMessage("product_limit must be one of: 4, 8, 12")
    .toInt(),
  body("sort_key")
    ...
    .withMessage(
      "sort_key must be one of: newest, rating, price_asc, price_desc",
    ),
  body("layout")
    .optional()
    .trim()
...
export const homepageCategoryProductSectionIdRules: ValidationChain[] = [
  param("id")
    .isUUID()
    .withMessage("Homepage category product section ID must be a valid UUID"),
];
...
const forbiddenHomepageCategoryProductFields: ValidationChain[] = [
  ...
  body("sort_order")
    .not()
    .exists()
    .withMessage("sort_order is not supported for category product sections"),
  body("metadata")
    .not()
    .exists()
    .withMessage("metadata is not supported for category product sections"),
  body("image_url")
    .not()
    .exists()
    .withMessage("image_url is not supported for category product sections"),
  body("background_image_url")
    .not()
    .exists()
    .withMessage(
      "background_image_url is not supported for category product sections",
    ),
  body("button_link")
    .not()
    .exists()
    .withMessage("button_link is not supported for category product sections"),
  body("link_url")
    .not()
    .exists()
    .withMessage("link_url is not supported for category product sections"),
];
...
const homepageCategoryProductSectionFields: ValidationChain[] = [
  ...
  body("product_limit")
    ...
    .bail()
    .custom((value) =>
      HOMEPAGE_CATEGORY_PRODUCT_LIMITS.includes(Number(value) as any),
    )
    .withMessage("product_limit must be one of: 4, 8, 12")
    .toInt(),
  body("sort_key")
...
export const trackingRules: ValidationChain[] = [
  ...
  body("tracking_url")
    ...
    .isLength({ max: 2048 })
    .custom((value) => !value || isSafeLink(value))
    .withMessage("Tracking URL must be safe"),
  body("estimated_delivery_date")
    .optional({ nullable: true })
    .isISO8601()
    .withMessage("Estimated delivery date must be valid"),
];
